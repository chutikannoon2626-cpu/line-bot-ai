...
const IMAGE_HANDOFF_MSG = 'รบกวนรอแอดมินตรวจสอบและติดต่อกลับนะคะ 🙏 หรือสอบถามข้อมูลเพิ่มเติมได้เลยนะคะ'
...
// (เรื่องที่ 99, เฉพาะ Facebook) ลิงก์แอดเพื่อน LINE OA ทางการ — ส่งเป็นข้อความแยกต่างหาก
// "มีแค่ลิงก์อย่างเดียว ไม่ปนข้อความอื่น" เสมอ เพราะทดสอบจริงแล้วว่า Facebook Messenger จะ unfurl
// เป็นการ์ดพรีวิว (รูป+ชื่อบัญชี+QR) ก็ต่อเมื่อข้อความมีแค่ลิงก์ล้วนๆ เท่านั้น — ไม่แก้ไขข้อความเดิม
// ที่มีคำว่า "@spenderclub" เลย (ของเดิมตั้งใจให้ลูกค้าอ่านแล้วพิมพ์ค้นหาเอง) แค่แถมข้อความนี้ต่อท้าย
// เป็นทางเลือกเสริมให้กดได้เลย
// (2026-09-08) เดิมใช้ลิงก์ย่อ 'https://lin.ee/ogHb2tW' — เจอเคสจริง: กดจากในแอป Messenger
// (in-app browser) แล้วไปต่อ "เปิด LINE" ไม่ได้เป็นบางครั้ง (ไม่คงเส้นคงวา) สันนิษฐานว่าลิงก์ย่อ
// ต้อง redirect ไปปลายทางจริงอีกที ทำให้ in-app browser ที่จำกัดสิทธิ์อยู่แล้วยิ่งมีโอกาสบล็อกการ
// เปิดแอปอื่นต่อกลางทาง — เปลี่ยนมาใช้ลิงก์ปลายทางจริงของ LINE OA ตรงๆ (ไม่ต้อง redirect ผ่าน
// lin.ee อีก) ยังไม่ยืนยัน 100% ว่าแก้ปัญหาถาวร เพราะพฤติกรรมที่เจอไม่คงเส้นคงวาตั้งแต่แรก
              ...
              if (alreadyRouted) {
                try {
                  const [count] = await redis.pipeline()
                    ...
                    .exec() as [number, number]
                  if (count === 1) {
                    await fbSend(psid, 'แอดมินจะรีบติดต่อกลับในเวลาทำการนะคะ 🙏\n🕐 เวลาทำการ 08:00–17:00 น. (จันทร์–เสาร์)')
                    log.info('fb.handoff.already_routed_ack', { userId })
                  } else {
                    log.info('fb.handoff.already_routed_silent', { userId, count })
                  }
