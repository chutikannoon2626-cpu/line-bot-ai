...
// เคลม/ส่งซ่อม — Web Chat ไม่มีแอดมิน real-time ให้เดินตาม repair_protocol ทีละขั้น
// ตอบให้ติดต่อแอดมินโดยตรงทันที ไม่ผ่าน Gemini (เฉพาะ Web Chat เท่านั้น ไม่กระทบ LINE/Facebook)
const CLAIM_RE  = /เคลม|ส่งซ่อม|claim/iu
const CLAIM_MSG = 'เรื่องเคลม/ส่งซ่อมรบกวนติดต่อแอดมินโดยตรงเลยนะคะ\nแอดไลน์ @spenderclub\nเบอร์โทรติดต่อ: 085-222-9111, 089-661-2682\nจันทร์–เสาร์ เวลา 08:00–17:00 น.ค่ะ'
...
function isNonsenseMessage(msg: string): boolean {
  ...
  // ตัวอักษรซ้ำ 3+ ติดกัน: ดดด, ไไไไ, !!! — ไม่นับตัวเลข ([^\d]) เพราะเลขซ้ำกันแบบนี้
  // เป็นเรื่องปกติมากในราคา/จำนวนสินค้า (เช่น "2,000" มี "000" ติดกัน) ไม่ใช่ข้อความไร้สาระ
  // (2026-08-18, เรื่องที่ 55 — เคสจริง: ลูกค้าถาม "...ตัวละ 2,000 นะครับ" ซ้ำๆ ถูกตัวกรองนี้
  // เข้าใจผิดว่าไร้สาระทุกครั้ง สกัดไว้ก่อนถึง Gemini/กฎกันถามซ้ำเลย ทำให้วนตอบไม่ตรงคำถามไม่จบ)
  if (/([^\d])\1{2,}/u.test(msg)) return true
  return false
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exactMatch = await findExactMatch(message, lastBotTurn)
    if (exactMatch) {
      try {
    ...
    // Case 3: ถามเรื่องเคลม/ส่งซ่อม → ตอบทันที ไม่ผ่าน Gemini (ข้าม repair_protocol)
    if (CLAIM_RE.test(message)) {
      await saveHistory(sessionId, [...history, { role: 'user', text: message }, { role: 'model', text: CLAIM_MSG }])
      log.info('webchat.claim_repair', { userId })
