  ...
  try {
    const res = await ai.models.generateContent({
      ...
      contents: [{
        ...
        parts: [
          {
            text: `ดูรูปทั้งหมดนี้ (${base64Images.length} รูป) แล้วตอบเป็น JSON เท่านั้น ไม่ต้องอธิบายเพิ่ม ห้ามแต่งข้อมูลที่ไม่เห็นในรูป ตามเงื่อนไขนี้:

1) ถ้าทุกรูปเป็นรูปสินค้าแบรนด์ SPENDER ตัวเดียวกันชัดเจน (วิทยุสื่อสาร, อุปกรณ์เสริม, อะไหล่, แท่นชาร์จ ฯลฯ) ระบุยี่ห้อ/รุ่นได้ชัดเจน (ไม่มีข้อความ error/คำเตือน หรือหน้าจอ/ฉลากแสดง IMEI ปนอยู่เลยสักรูป):
{"kind":"product","product":"Spender TC-4M"}

2) ถ้าเป็นรูปอื่นๆ ที่พอมองออกว่าเป็นอะไร (ไม่ว่าจะเป็นแบรนด์ไหน สินค้าประเภทไหน หรือสถานการณ์แบบใดก็ตาม เช่น สินค้า/อุปกรณ์แบรนด์อื่นที่ไม่ใช่ SPENDER, หน้าจอ/ข้อความ error หรือคำเตือนบนตัวเครื่อง, ฉลากหรือหน้าจอแสดงเลข IMEI, สกรีนช็อตแอปหรือเว็บไซต์, เอกสารหรือใบเสร็จ ฯลฯ) **หรือมีมากกว่า 1 รูปและแต่ละรูปเป็นคนละสินค้า/คนละเรื่องกัน** — ให้สรุปสั้นๆ ว่าเห็นอะไร (ภาษาไทย) แล้วแต่งเป็นประโยคคำถามยืนยันกับลูกค้า โทนสุภาพเป็นมิตรแบบผู้หญิง ลงท้ายด้วย "ค่ะ"/"คะ" เสมอ ถามว่าต้องการให้ช่วยเรื่องนี้ใช่ไหม ห้ามตอบคำถามหรือแนะนำวิธีแก้ใดๆ เอง แค่สรุป+ถามยืนยันเท่านั้น เช่น:
  ...
  try {
    const res = await ai.models.generateContent({
      ...
      contents: [{
        ...
        parts: [
          {
            text: `ดูรูปทั้งหมดนี้ (${base64Images.length} รูป) พร้อมข้อความที่ลูกค้าพิมพ์มาด้วย: "${userText}"
